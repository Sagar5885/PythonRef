            ...
            try:
                ...
                try:
                    if (response_ip_str['basic']['isOutOfStock'] is None):
                        Errors.append(
                            pr_id + " " + st_id + " " + "Item page Fail to get respective product isOutOfStock is not available!")  # not able to handle it
                    else:
                        if (response_ip_str['basic']['isOutOfStock'] == True):
                            try:
                                ...
                                headers_iro = {'WM_SVC.VERSION': '2.0.0',
                                               ...
                                               'WM_VERTICAL_ID': '2'}
                                response_iro = requests.request("POST", url_iro, data=req_payload_iro,
                                                                headers=headers_iro)
                                response_iro_str = response_iro.json()

                                if (response_iro_str['status'] == 'OK'):
                                    print(response_iro_str['payload'][0]['offerWrappers'][0]['storeFronts'][0][
                                              'canAddToCart'])
                                    if (response_iro_str['payload'][0]['offerWrappers'][0]['storeFronts'][0][
                                            'canAddToCart'] == False):
                                        print("Item Page (IP-OOS-FALSE) & IRO Mismatching (IRO-OOS-TRUE)" + pr_id)
                                        Errors.append(
                                            pr_id + " " + st_id + " Item Page (IP-OOS-FALSE) & IRO Mismatching (IRO-OOS-TRUE)")
                                else:
                                    Errors.append(pr_id + " " + st_id + " IRO is Status is " + response_iro_str[
                                        'status'] + " Error Code: " + response_iro_str['payload'][0]['entityErrors'][0][
                                                      'code'])
...
if __name__ == '__main__':
    with Pool(5) as p:
        p.map(groceryStatusCk, ['test2.csv'])